...
def Teff(t,X_nickel):
    #equation 41
    #15493 Kelvin * M_ni^*(0.25) * v_9^(-0.5) * (Lambda(x,y)/x^2/y^2)^(0.25)
    #note that xy ~ t/2t_nickel = t/17.6 days,
    #and that x = t/t_m, where t_m is the geometric mean of the expansion and diffusion timescales.
    #and that y = t_m/2t_nickel

    #they tabulate Lambda(x,y)
    #from their equ 45, small xy (before peak) means that
    #Lambda(x,y) ~ x^2, i.e., fireball behavior.
    #using equation 44 to approximate
    #time is in days,
    #M_ni = mass of nickel_56 in solar masses
    #v_9 = velocity in 10^9 cm/s
    #x = dimensionless radius = r(m,t)/R(t)
...
def L_bol(t,X_nickel):
    #equation 36
    #L(1,t) = e_ni * M_ni * (Lambda(x,y)
    #note that xy ~ t/2t_nickel = t/17.6 days,
    #and that x = t/t_m, where t_m is the geometric mean of the expansion and diffusion timescales.
    #and that y = t_m/2t_nickel

    #In other words, Lambda is the shape of the light curve, scalled to twice the nickel half-life
    #they provide tables for different shapes that corresond to different y's (which are really controlled by t_m)
