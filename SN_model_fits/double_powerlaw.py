...
def double_powerlaw(tin,params):
    ...
    out[m] += params['norm']*(  (tin[m] - params['t_explosion'])  )**params['index1']

    #above tbreak, change the exponent
    m = (tin > tbreak)
    out[m] += params['norm']*(  tin[m] - tbreak  )**params['index2']
    
    return out


def fit_double_powerlaw_to_single_sector(times, fluxes,efluxes):
    ...
    pinit.add('delta_t', value = 5.0,   min = 0.0, max = 20.0)
    pinit.add('norm',  value= 1.0)
    pinit.add('index1', value= 1.0,min=0.0, max=4.0)
    pinit.add('index2', value= 2.0,min=0.0, max=4.0)
...
#    m2fit = time2 < time2[100]
    ...
    mresult = minimize(merit, pinit, args = (double_powerlaw,
                                             time0, flux0, eflux0),                                                                                 
                       maxfev=500000 )
    return mresult


def double_powerlaw_with_calibration(tin, params):
    #fits for a time of explosion and a power-law rise
    ...
    out1[m] += params['norm']*(  tin1[m] - params['t_explosion']  )**params['index1']
    m = tin1 > tbreak
    out1[m] += params['norm']*(  tin1[m] - tbreak  )**params['index2']

    #fit second sector
    m = tin2 > params['t_explosion']
    out2[m] += params['norm']*(  tin2[m] - params['t_explosion']  )**params['index1'] 
    m = tin2 > tbreak
    out2[m] += params['norm']*(  tin2[m] - tbreak  )**params['index2']
    out2[m] += params['offset']

    
    return sp.r_[out1,out2]


def fit_double_powerlaw_to_two_sectors(times, fluxes, efluxes):
    ...
    pinit.add('delta_t',value = 5.0, min = 0.0, max = 10.0)
    pinit.add('norm',  value= 1.0)
    pinit.add('index1', value= 1.0,min=0.0, max=4.0)
    pinit.add('index2', value= 2.0,min=0.0, max=4.0)
...
#    m2fit = time2 < time2[100]
    ...
    mresult = minimize(merit, pinit, args = (double_powerlaw_with_calibration,
                                             [time1, time2],
                                             sp.r_[flux1, flux2],
                                             sp.r_[eflux1,eflux2]),
                       maxfev=500000)
